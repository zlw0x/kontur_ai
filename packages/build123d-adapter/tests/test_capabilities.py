...
def test_a_plain_plate_asks_for_the_plainest_things():
    needed = caps.requirements(document("plate.v1_9.json"))
    assert set(needed) == {
...
def test_the_lever_plate_asks_for_everything_it_actually_uses():
    ...
    needed = caps.requirements(document("lever-plate.v1_9.json"))
    assert set(needed) == {
        caps.SOLID_RECTANGULAR_PRISM,
...
def test_a_second_additive_feature_is_a_boss_and_the_first_is_not():
    ...
    """
    assert caps.FEATURE_BOSS_ADDITIVE not in caps.requirements(document("plate.v1_9.json"))
    assert caps.FEATURE_BOSS_ADDITIVE in caps.requirements(document("lever-plate.v1_9.json"))


def test_the_bushing_asks_for_revolve_and_for_the_revolved_cut():
    needed = caps.requirements(document("bushing.v1_9.json"))
...
def test_a_disabled_feature_asks_for_nothing():
    ...
    value = json.loads((FIXTURES / "bushing.v1_9.json").read_text("utf-8"))
...
def test_the_bracket_asks_for_each_blend_separately():
    ...
    """
    needed = caps.requirements(document("blended-bracket.v1_9.json"))
...
def test_turning_off_convexity_refuses_the_document_before_any_geometry():
    ...
    with pytest.raises(CadEngineError) as refused:
        gate.require_all(caps.requirements(document("blended-bracket.v1_9.json")))
...
def test_the_boolean_bracket_asks_for_a_key_per_operation():
    ...
    to stop only that.
    """
    needed = caps.requirements(document("boolean-bracket.v1_9.json"))
...
def test_the_flange_asks_for_each_kind_of_pattern_separately():
    ...
    """
    needed = caps.requirements(document("patterned-flange.v1_9.json"))
...
def test_turning_off_one_kind_of_pattern_refuses_the_document_whole():
    ...
    with pytest.raises(CadEngineError) as refused:
        gate.require_all(caps.requirements(document("patterned-flange.v1_9.json")))
    assert refused.value.code == "CAPABILITY_DISABLED"
...
@pytest.mark.parametrize(
    "name",
    ["plate.v1_9.json", "plate-with-hole.v1_9.json", "constrained-plate.v1_9.json",
     "lever-plate.v1_9.json", "bushing.v1_9.json", "blended-bracket.v1_9.json",
     "patterned-flange.v1_9.json", "boolean-bracket.v1_9.json"],
)
...
def test_a_document_needing_a_disabled_operation_is_refused_whole():
    ...
    with pytest.raises(CadEngineError) as refused:
        gate.require_all(caps.requirements(document("lever-plate.v1_9.json")))
    assert refused.value.code == "CAPABILITY_DISABLED"
...
def test_turning_off_something_the_document_does_not_use_changes_nothing():
    ...
    gate.require_all(caps.requirements(document("plate.v1_9.json")))
