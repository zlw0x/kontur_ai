...
public sealed class ResourceLedgerTests
{
    ...
    public void UnknownCodexUsageIsReportedAsUnknownNotAsZero()
    {
        ...
        var usage = result.ToAiUsage("drawing-mvp-3");
    ...
    public void MeasuredCodexUsageCarriesItsSourceAndCounts()
    {
        ...
        Assert.Equal("drawing-mvp-3", usage.PromptVersion);
    ...
    public async Task DrawingPipelineRecordsAnAiRunPerCodexStage()
    {
        ...
        try
        {
            ...
            var valid = await File.ReadAllTextAsync(
                Path.Combine(FindRepositoryRoot(), "tests", "fixtures", "cad-ir", $"plate.{CadIr.FileSuffix}.json"));
    ...
    {
        var report = await new FakeDocumentEngine(CadIr.Version).DescribeAsync([], CancellationToken.None);
        ...
        Assert.Equal([CadIr.Version], manifest.CadIrVersions);
