...
def test_a_worker_that_stopped_heartbeating_goes_stale():
    ...
    protocol.heartbeat(worker, [WorkerCapability.KOMPAS_BUILD], ["0.1.0"], 1, manifest())
    job = build_job()
    protocol.enqueue(job)
...
def test_an_unsupported_capability_names_the_capability():
    ...
    protocol.heartbeat(worker, [WorkerCapability.KOMPAS_BUILD], ["0.1.0"], 1, incomplete)
    job = build_job()
    protocol.enqueue(job)
...
def test_a_busy_but_capable_worker_reports_capacity_not_incapability():
    protocol, worker, diagnostics, _ = fixture()
    protocol.heartbeat(worker, [WorkerCapability.KOMPAS_BUILD], ["0.1.0"], 0, manifest())
    job = build_job()
    protocol.enqueue(job)
...
def test_one_capable_worker_makes_the_job_claimable_despite_an_incapable_peer():
    ...
    protocol.heartbeat(capable, [WorkerCapability.KOMPAS_BUILD], ["0.1.0"], 1, manifest())
    protocol.heartbeat(incapable, [WorkerCapability.KOMPAS_BUILD], ["0.1.0"], 1)
    job = build_job()
    protocol.enqueue(job)
