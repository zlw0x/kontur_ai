...
> **Историческая справка (2026-08-09).** Разделы 1–2 описывают исходную точку
> июля 2026 года, когда движком был КОМПАС-3D, а продукт отдавал M3D. С ADR-023
> ничего этого нет: строит **build123d на OpenCascade в Linux-контейнере**, а
> продукт — это STEP и STL. Текст ниже оставлен как есть, потому что он объясняет,
> из какой точки вышли решения ниже по документу; читать его как описание
> сегодняшнего сервиса нельзя. Что сервис делает сейчас — `CLAUDE.md` и
> `docs/adr/ADR-023-*`.
>
> Правило репозитория: комментарии, объясняющие **почему** что-то так, остаются
> всегда; описания того, чего больше нет, — помечаются или уходят.

## 1. Исходная точка (июль 2026, до ADR-023)
...
3. fake adapter;
4. реальный адаптер движка (тогда KompasAdapter, теперь build123d);
...
Граница не изменилась с переходом на build123d — изменились только два последних
звена (`build123d-adapter` и контейнер с OpenCascade). Это и было смыслом CAD-IR
как границы доверия: движок под ней можно заменить.
...
Корпус — **100 положительных случаев и 42 отрицательных** на CAD-IR 1.15. Планка P1
(50 фикстур × 20 наборов параметров) ещё не взята: корпус строит каждую форму на двух-трёх
наборах, а не на двадцати.
...
**Взято, 2026-08-12** (`docs/acceptance/POSTMVP-028-the-corpus-reaches-gate-p2.md`).
Все три числа: **100 моделей, 38 типов деталей, 100 из 100 детерминированных сборок.**
Тридцать пять новых случаев в пятнадцати новых типах деталей — шайба, втулка, ступенчатый
вал, фланец, прокладка, шестигранник, уголок, призма с V-пазом, крестовина, крышка с
цековкой, косынка, планка с пазами, гнутый фланец — и ни один из них не новая операция.

Тридцать два из тридцати пяти сошлись с первой сборки; три упавших — одна и та же ошибка
зависимости, которую поймал валидатор контракта.

Детерминизм сперва дал 99 из 100, и исключение стоило разбора: в STEP-сборке OpenCascade
нумерует вхождения счётчиком, который живёт **в процессе**, а не в документе. Два
отдельных процесса дают побайтово одинаковые STEP и STL, включая метки, — а воркер
собирает одно задание на процесс. Метка нормализуется, как и `FILE_NAME`, и отдельный
тест держит исключение узким: путь вхождения и обе ссылки на сущности меняться не имеют
права.

Что это продвинуло: **восемнадцать ключей стали `stable`** по правилу, которое проверяется
в обе стороны — возможность становится `stable`, когда корпус строит её **минимум на трёх
разных типах деталей**. Три, а не один, потому что дефекты этого репозитория ровно такие:
многотельная бобышка из POSTMVP-006 и claim с двумя группами отверстий, который
POSTMVP-027 нашёл на фланцах после девяти прогонов на деталях с одним размером отверстия.
...
> **Спроектировано, не построено** (`docs/TASK-POSTMVP-P3-2-up-to-a-face.md`, 2026-08-04).
> Единственное, чего не хватало, — «до следующей грани», и исследование заканчивается тем,
> что `extrude(until=…)` не нужен вовсе. Шестнадцать случаев на блоке 40 × 40 × 10: два
> верны, три падают, а три **успешно врут** — профиль внутри материала выстреливает шипом
> до z = 62.45 (это `find_max_dimension`, а не чертёж) и сообщает об одном валидном теле;
> вырез «до следующей поверхности» может не убрать ничего. И `until` — первая операция,
> которая не заявляет **никакого числа**, поэтому пост-проверка, поймавшая три последних
> дефекта, сравнивать ей не с чем.
>
> Поэтому выдавливание до грани **называет грань селектором** (то же правило ADR-019), а
> доверенный код считает вылет: `((p − o)·n)/(d·n)`. Измерено: совпадает с ответом ядра
> до 0.000e+00 там, где `until=` работает, даёт корпусу замкнутую форму, которую ядро
> держало при себе, а каждый из отказов превращается либо в отказ контракта, либо в два
> тела, которые уже видит `body_count`. Толщина ребра — это `both_directions` с 1.10, уклон
> — `taper_deg` с неё же. Реализация — версия CAD-IR.
>
> **Сделано** (CAD-IR 1.13, ADR-039). Не хватало только вылета, и он приехал как
> `until_face`. Само ребро операции не требует вовсе (POSTMVP-022): замкнутый
> контур, выдавленный в обе стороны, 31 468.0000 мм³ против замкнутой формы
> 31 468 — фикстура `rib-bracket` строит именно это.

## P3.3. Draft

> **Сделано** (POSTMVP-026, `docs/adr/ADR-035-*`, CAD-IR 1.12). Операция вошла против
> правила, к которому пришли три вехи, и поэтому обязана была сказать то, чего композиция
> не говорит. POSTMVP-024 измерил, что уклонённый бобышка — **тот же самый** объём (26
> 689.1761 мм³) хоть `extrude(taper=10)`, хоть уклон стенок после. Аргументом стали два
> других замера: **две стенки из четырёх** (29 178.7680 мм³, `a·h·(a − h·tanθ)`, габарит
> не меняется, потому что две неуклонённые стенки стоят там, где их поставил чертёж) и
> **тело, которого не делало выдавливание** (наружная стенка точёной втулки, 18 849.5559
> → 14 678.4446 — усечённый конус минус отверстие). Обе замкнутые формы точны.
>
> Грани называет селектор, и нейтральную грань тоже: её плоскость держит размер. Нормаль
> разворачивается **внутрь материала** — иначе плюсовой угол сужал бы деталь вниз, потому
> что нижняя грань смотрит наружу. Развёрнутая, она даёт один и тот же ответ, какой бы
> конец документ ни назвал, и правило формулируется одной фразой: *плюс сужает стенки по
> мере ухода от нейтральной грани*.
>
> Два «впервые» в отказах: ровно на угле смыкания ядро возвращает пирамиду и **само
> говорит `is_valid` false** (до сих пор все находки этого рода приходили с уверенным
> «валидно»), а дальше бросает `Standard_ConstructionError` **с пустым сообщением**. Оба —
> `DRAFT_TOO_STEEP`.
...
> **Измерено, 2026-08-12** (`docs/TASK-POSTMVP-P3-4-a-thread-is-a-direction.md`,
> проба `scripts/probe_build123d_thread.py`). `thread.modeled` **не требует ни новой
> операции, ни версии контракта**: резьба — это профилированное сечение по винтовой
> траектории и вычитание, то есть композиция, и правило POSTMVP-011 отказывает ей в
> собственном типе. Gate P3 спрашивает «проходит ли modeled thread manifold check» —
> M20×2.5 на 2, 6 и 12 витков: **0 открытых рёбер, 0 перевёрнутых нормалей, род 0**,
> 0.13–1.68 с и 112–578 КиБ. Внутренняя тоже проходит.
>
> Но сначала пришлось поправить движок, и это не про резьбу. Плоскость сечения у
> винтовой траектории задавала только **нормаль**, а рамку внутри плоскости выбирал
> build123d по эвристике. Круглому сечению всё равно — потому 1.14 этого и не увидел, —
> а у резьбы фланки это и есть направление: то же сечение, повёрнутое, снимает
> **188.34 мм³ вместо 374.19**, без единой ошибки. Теперь рамку строит движок из самой
> траектории: x — ось винта, спроецированная в плоскость сечения, y — радиально наружу.
>
> И заметание идёт кадром Френе: рамка по умолчанию **закручивает сечение** — 0.052%,
> 0.476%, 1.733%, 4.745% на 1, 3, 6 и 12 витках, — а под Френе замкнутая форма
> `V = A·L·(1 − κ·ū)` точна на любом числе витков. Круглое сечение стоит на самой
> траектории, поэтому `ū = 0` и вся поправка исчезает: арифметика пружины из 1.14 была
> частным случаем.
>
> Осталось **`thread.designation`** — надпись, а не геометрия, и это ровно то, что
> POSTMVP-011 назвал единственным настоящим пробелом. Её нельзя проверить измерением
> детали: M20×2.5-6g — не форма, и деталь несёт эту надпись хоть с верным допуском, хоть
> с неверным. Место в конвейере из этого и следует: читающая стадия снимает её с чертежа,
> claim несёт, а поймать ошибку может только оператор. Это версия контракта.
...
> **Отказано, 2026-08-12.** Ровно потому и отказано: они компилируются в то, что
> контракт уже умеет. Это тот же довод, которым POSTMVP-011 отказал семействам
> отверстий, а POSTMVP-022 — ребру: второй способ сказать то, что CAD-IR говорит
> один раз, и каждый лишний тип — ещё одна вещь, которую надо валидировать.
> «Упрощает reasoning модели» — это довод за промпт и выходной профиль (ADR-029,
> ADR-032), а не за контракт.
...
Вторая половина гейта **закрыта и измерена** (`docs/TASK-POSTMVP-P3-4-*`): M20×2.5
на 2, 6 и 12 витков — 0 открытых рёбер, род 0; внутренняя резьба тоже. Первая
половина — про сквозной цикл, а не про геометрию, и упирается в зрение (ADR-029).
...
**Разобрано, 2026-08-12** (`docs/TASK-POSTMVP-P4-3-a-path-that-leaves-its-plane.md`,
ADR-041). Это не одна стена, а пять разных вопросов, и только один из них вообще про
координаты.

| пункт | итог | почему |
|---|---|---|
| 3D polyline | **сделано** (1.15) | одно число на точку; всё остальное — правила 1.9 |
| цилиндрический винт | сделано (1.14) | пять чисел и направление, ADR-040 |
| конический винт | **сделано** (1.15) | одно поле — и `pitch` у ядра меряется по образующей, а на чертеже вдоль оси |
| 3D spline | **отказ** | заявленные точки не задают кривую, и кривая выходит за коробку этих точек |
| intersection / projected curve | **отказ** | не заявляет числа — довод ADR-039 против `until` |
| imported points | **отказ** | форма детали оказывается вне её документа, а хэш обязан её опознавать (ADR-018) |

Чего это стоило кроме контракта: **Паппус точен и для пути в пространстве** (поправка —
первый момент сечения относительно пути, а он ноль), поэтому объём вообще не видит
третьего измерения и его видит габарит. И найдена дыра в 1.9: **путь, который
возвращается рядом с собой, проходит все проверки сервиса** — отсюда
...
> **Отказано, 2026-08-12** (`docs/TASK-POSTMVP-closing-the-remaining-stages.md`, проба
> `scripts/probe_build123d_remaining_stages.py`). Причина — измерена, а не выведена:
> у грани **площадь 2400.0000 и объём 0.0000**, а её STL — 2 треугольника с 4 открытыми
> рёбрами. Теперь список того, чем этот сервис проверяет деталь: объём, габарит,
> `body_count`, `closed_manifold_mesh`, `consistent_normals`, сквозная сверка рода и
> shape claim. **Каждая из этих проверок — вопрос о теле.** Пустить поверхности значит
> отдавать деталь, которую не проверяет ничто, — ровно наоборот тому, ради чего писались
> ADR-018…ADR-025.
>
> Ядро тут ни при чём: `thicken(face, 3)` даёт **7200.0000**, точно, 0 открытых рёбер.
> Дело в том, что CAD-IR нечем *сказать* поверхность, и даже эта единственная операция
> требует всего словаря, ради которого отказан остальной P5, — а он размером с текущий
> контракт. Операции, которые кончаются телом, уже есть: loft по сечениям, sweep по
> траектории, revolve. Именно ими здесь и строится кривая деталь.
>
> Что откроет заново: чертёж, деталь которого нельзя выразить телом из тел. За 100
> размеченных заказов (POSTMVP-027) и все приёмочные прогоны такого не появилось.
...
> **Разобрано, 2026-08-12.** Согнутая деталь **уже строится**: 2 мм лист шириной 60,
> согнутый на 90° по внутреннему радиусу 3, — это прямоугольное сечение по траектории
> прямая-дуга-прямая. Объём 9393.9822 против замкнутой формы 9393.9822 (расхождение
> 5.5e-12), 0 открытых рёбер, одно тело. **Равнотолщинность здесь не проверка, а свойство
> построения** — сечение по пути не меняется. Столкновения ловят
> `SOLID_PASSES_THROUGH_ITSELF` и правило зазора на изгибе, оба уже работают.
>
> Осторожность, стоящая 140 открытых рёбер: тот же самый sweep, написанный **напрямую**
> к build123d, разваливается — сечение 60 × 2 так же чувствительно к рамке внутри
> плоскости, как фланки резьбы (поправка к ADR-040). Движок цел только потому, что
> CAD-IR требует **заявить** плоскость профиля, а не унаследовать её.
>
> Настоящей работы две, и обе не про геометрию. **K-фактор ничего не меняет в отданном
> теле**: при K = 0.33 / 0.42 / 0.50 развёртка 75.7491 / 76.0319 / 76.2832, а объём один
> и тот же — три заготовки, одно тело. Значит толщина, радиус гиба, K-фактор и тип
> отжима — это тот же тип изменения контракта, что `thread.designation`: снимается с
> чертежа, несётся claim, показывается оператору, измерением детали не проверяется.
> И **развёртка с экспортом DXF** — единственное по-настоящему новое вычисление,
> плюс третий артефакт рядом со STEP и STL. Делать её имеет смысл **после** словаря:
> развёртка по K-фактору, которого никто не заявил, — число, которое никто не выбирал.
...
> **Разобрано, 2026-08-12.** Единственный проектный вопрос этапа — решает ли сопряжения
> решатель — закрыт правилом ADR-022, которое старше этапа: **ограничение это утверждение
> о координатах, которые заявил документ, и никогда не инструкция, которая их порождает.**
> Значит сборка здесь — это тела, расставленные документом, плюс проверки расстановки, а
> coincident/concentric/distance/angle — это expectations, а не решатель.
>
> Пересечение измеряется объёмом: два куба 20 мм с центрами в 26 мм дают **0.0000**, с
> центрами в 16 мм — **1600.0000**, ровно 20 × 20 × 4. Это арифметика над телами, которые
> документ уже расставляет (`new_body`, `source_body`, `feature.boolean` — с CAD-IR 1.7).
>
> Остаётся **упаковка, а не геометрия**: STEP и STL на деталь (каждое тело уже
> экспортируется отдельно), сборочный STEP (несколько тел уже уходят компаундом), ZIP,
> expectation `interference` размером с `body_count`, и разнесённый предпросмотр —
> функция просмотрщика без геометрии за ней.
...
> **Большая часть приехала под другими именами** во время производственного аудита
> (`docs/TASK-POSTMVP-closing-the-remaining-stages.md`): concurrency = 1 — умолчание
> воркера; Windows Job Object **устарел** (CAD живёт в Linux-контейнере, ADR-023);
> watchdog и принудительная уборка — launcher владеет дочерним процессом и закрывает его
> stdin на старте; потерянные аренды и сироты — reaper из P0-3, `LEASE_LOST` и
> `JobStatus.PAUSED`; дисковая квота — карантин считает и **останавливается** на пределе
> (P0-2); бюджеты — `CODEX_BUDGET_EXHAUSTED`, `max_attempts` и квоты заказа из P1-7.
> Политика перезагрузки не построена и является runbook оператора, а не кодом.
>
> **Quality score — то, что стоит собрать следующим**, и он мал, потому что все числа в
> нём уже существуют в конвейере: счётчик ремонтов, счётчик допущений, отчёт верификации,
> вердикт claim, был ли раунд уточнений. Это сборка, а не исследование, и у неё есть
> заказчик: очередь модерации станет сортируемой вместо просматриваемой.
> `automatic_acceptance` остаётся выключенным в любом случае (P0-5) — 2 неверные детали
> на 100 без единого слова это и есть то, ради чего нужен человек.
...
6. Rib и draft (P3.2, P3.3): уклон в геометрии с 1.10, слово для него в claim — с
   2026-08-04; rib спроектирован (`docs/TASK-POSTMVP-P3-2-up-to-a-face.md`) и ждёт версии
   контракта. По Gate P4 два небольших куска сделаны (`docs/GATE-P4-ANALYSIS.md`), дальше
   стена P4.3 — 3D-кривые.
7. Арифметика в `Scalar` — параметр с лучшей провенансой ничего не двигает
   (`docs/TASK-POSTMVP-scalar-arithmetic.md`): форма выбрана и собрана, включение в
   `Scalar` — версия контракта.
8. Калибровка тарифного профиля — он до сих пор поставляется с нулевыми ставками.
