...
Корпус — **65 положительных случаев и 42 отрицательных** на CAD-IR 1.15. Это не планка
P1 и не планка P2, и именно поэтому ничто в `capabilities.py` не объявлено `stable`.
Закрытие обоих гейтов — работа над корпусом, а не над движком: больше форм, больше
наборов параметров, и арифметика каждого в замкнутой форме.
...
Самый дешёвый из оставшихся пунктов и самый влиятельный: именно он переводит
возможность из `beta` в `stable`, то есть в то, что оператор может выдавать в аренду.
