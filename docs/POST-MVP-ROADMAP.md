...
### Контейнер: что готово и почему сборка не прогнана здесь

Образ (`apps/cad-worker/Dockerfile`) по-прежнему ни разу не собран и деплой на нём не
запускался — это последний хвост миграции. Что изменилось: он стал **проверяемым, а не
просто описанным**.
