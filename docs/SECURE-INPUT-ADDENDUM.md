...
| изолированный процесс с `RLIMIT_AS`, `RLIMIT_CPU` и стенными часами | **сделано** на POSIX |
| платформа без POSIX-лимитов (Windows): процессный режим отказывается работать вне `local` | **сделано** |
| контейнерный режим (network none, read-only, cap-drop ALL, no-new-privileges, memory/CPU/PID) | **сделано**: образ собран, бомба и обычная страница прогнаны через него (2026-08-09), джоба `sanitizer-image` в CI |
...
| immutability на уровне БД и object storage | частично: заказы в PostgreSQL с 0008, но артефакты в object storage — нет |
...
### 0.2. Контейнер собран и прогнан (2026-08-09)

Образа не было, и это значит, что §2.2 существовал как argv, который никто не
запускал, — то есть как описание намерения. Теперь `packages/image-sanitizer/Dockerfile`,
`cad-ai/sanitizer:1`, и джоба `sanitizer-image` в CI, которая его собирает и прогоняет
через него **настоящую бомбу**, а не проверяет строку.

Измерено на этой машине:

```text
обычная страница 8×6  -> {"ok": true, "width": 8, "height": 6, "limits": "rlimit"}
бомба 30000×30000     -> {"ok": false, "code": "INPUT_PIXELS_TOO_MANY", "limits": "rlimit"}
                         2.6 МБ на диске, ~2 ГиБ в развёрнутом виде
```

Отказ приходит **до** декодирования, файла после себя не оставляет, и `"limits":
"rlimit"` подтверждает, что внутри контейнера потолки ядра тоже стоят — контейнер
их не заменяет, а добавляет к ним пространство имён. Процессный режим ограничивает,
сколько декодер может *потратить*; только контейнер ограничивает, что он может
*видеть*.

Сторож в джобе читает ответы, а не код возврата. Первая версия соседней джобы была
написана через `grep -qv skipped`, что успешно всегда: `-v` инвертирует строки, а не
результат, и любая строка, кроме названной, — успех. Так джоба зеленела, не выполнив
ничего.

### 0.1. Четвёртое состояние процессного режима (2026-08-09)

Режимов было описано три — контейнер, процесс, «никогда в API». Их оказалось
четыре, и четвёртый нашёлся не рассуждением, а тем, что на машине оператора
пятнадцать тестов стали красными.

`RLIMIT_AS` и `RLIMIT_CPU` — это POSIX. `import resource` стоял на уровне модуля,
поэтому на Windows санитайзер **не запускался вовсе**: API получал пустой stdout и
для каждой загрузки отвечал `SanitizerUnavailable`. Вердикт верный, причина не та —
читается как «санитайзер сломан», а не «эта платформа не умеет его ограничивать».

Сделать импорт необязательным и на этом остановиться было бы худшим из починок:
программа запустилась бы, раскодировала чужой файл под одними стенными часами и
сказала бы ровно то же самое, что говорит при всех выставленных потолках. **Молчащее
ослабление вместо шумного отказа.**

Поэтому: ребёнок сообщает в своём JSON, какие потолки он реально получил
(`"limits": "rlimit"` или `"none"`), причём и в отказах тоже — декодирование без
ограничений остаётся декодированием без ограничений независимо от того, понравился
ли декодеру файл. А эта сторона спрашивает дважды и по-разному:

- **до** того, как файл передан: есть ли на этой машине POSIX-лимиты вообще;
- **после** ответа: совпадает ли то, что сказал процесс, с тем, что требовалось.

Разрешён этот режим только в окружении `local` — на ноутбуке, который не умеет
Docker, сервис должен запускаться. Вне `local` у оператора есть контейнер, который
нужно настроить, и `sanitizer_image` перестаёт быть удобством. Форма та же, что у
`reject_development_secrets_outside_local`.

Побочно найдено и починено там же: пустое окружение ребёнка (он не должен видеть
наш `DATABASE_URL`, токены и `PATH`) забирает и те переменные, по которым
интерпретатор находит **свои собственные** сторонние пакеты — `APPDATA` на Windows,
`HOME` на POSIX. Pillow, установленный в user site, становился невидим. Путь
импорта теперь вычисляется и передаётся списком каталогов: окружение остаётся
пустым, а ответ — одинаковым на любой машине.
